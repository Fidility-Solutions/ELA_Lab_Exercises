/*
 * File       : condition_variable_static.c
 *
...
static void* threadfunc(void *args){
	...
	printf("Thread_%d acquired lock \n",ThreadID);
	printf("Thread_%d accessed shared resource\n",ThreadID);
	/* Inc variable */
	NumUnJoind++;
...
int main(int argc,char *argv[]){
	...
	if(argc<2 || strcmp(argv[1],"--help") == 0){
		printf("Syntax		: filename thread1sleeptime thread2sleeptime ...threadnsleeptime\n\n");
		...
		printf("...\n");
		printf("threadnsleeptime: sleeptime of threadn\n");
	...
	while(NumLive>0){
		...
		while(NumUnJoind ==0){
			printf("\nMain thread mutex lock is released due to the wait condition\n");
			...
			printf("\nMain thread acquired the lock after receiving conditional signal from thread\n");
			if(s!=0)
		...
		for(index=0;index<TotThreads;index++){
			if(ThreadArray[index].state == TS_TERMINATED){
				...
				NumLive--;
				NumUnJoind--;
				printf("Reaped Thread_%d(NumLive =%d)\n",index,NumLive);
			}
		}
		...
		printf("Main thread released the mutex\n");
	}
