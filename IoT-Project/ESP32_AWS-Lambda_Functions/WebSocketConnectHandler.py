...
def lambda_handler(event, context):
    # Get connection ID from event context
    connectionId = event['requestContext']['connectionId']
    
    # Safely get location from query string parameters, default to 'Unknown'
    ...
    item = {
        "connectionId": {"S": connectionId},  # Correct format for partition key
        "location": {"S": location}  # Store location (defaulted to 'Unknown' if missing)
    }

    print(f"Formatted DynamoDB Item: {item}")  # Log formatted item for debugging

    try:
        # Insert the item into DynamoDB
        dynamodb.put_item(
            TableName="WebsocketConnections",
            Item=itemimport json
import boto3
import os
import time
