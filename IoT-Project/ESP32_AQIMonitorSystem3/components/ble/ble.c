...
STR_BLE_PARAM_CONFIG str_ble_param_config;
...
static void parse_wifi_credentials(const char* ps8Data, uint64_t u64Len, STR_WIFI_CREDENTIALS* pstrpCrntls) 
{
	...
	*ps8Delimiter = '\0';
	...
	/* Manual configure */
	wifi_init(pstrpCrntls->ssid, pstrpCrntls->password); 
}
...
static void parse_location(const char* ps8Data, uint64_t u64Len, STR_LOCATION* pstrLoc)
{
	...
	strcpy(str_ble_param_config.as8blelocation, as8Location);
	strcpy(str_ble_param_config.as8bleDeviceId, as8DeviceName);

	ESP_LOGI(BLE_TAG, "Parsed and Stored Location: %s", str_ble_param_config.as8blelocation);
	ESP_LOGI(BLE_TAG, "Parsed and Stored Device Name: %s", as8DeviceName);
}
...
static void apply_hardware_version(const char *hw_version)
{
	...
	strcpy(str_ble_param_config.ps8bleHVersion, hw_version);
}
...
static void apply_software_version(const char *sw_version)
{
	...
	strcpy(str_ble_param_config.ps8bleSVersion, sw_version);
}
