 ...
 * Date		: 08/05/2024
 *
 * Reference	:   [Any relevant references or sources]
*******************************************************************************/
 ...
 *
 * Description	: This function sets the direction of the specified GPIO pin (input or output) by writing to the /sys/class/gpio/gpio<GpioPin>/direction file.
 *
 ...
 * Function	: ToggleLed
 *
 * Description	: This function toggles the LED on the specified GPIO pin. It writes "1" to the pin's
 ...
 *                the user to select an LED to toggle, and toggles the selected LED accordingly.
 *                It continues to prompt the user until the program is terminated.
 *
 * Returns	:     0 upon successful execution of the program.
 */
int main(void) {
	printf("This program will demonistrates working with GPIO pins to Blink RGB Leds\n");
    	int choice;
	...
	for(int i=0;i<NUM_PINS;i++){
    	...
    	printf("Enter LED number (1, 2, 3 or 4 for to toggle all LEDs):");
	while(1){
    		scanf("%d", &choice);
    		switch(choice){
    	    	case 1:
    	    		printf("%d:Green led toggle 3time\n",choice);
			fflush(stdout);
    	        		ToggleLed(PIN_1);
    	        		break;
    	    	case 2:
    	    		printf("%d:Red led toggle 3times\n",choice);
			fflush(stdout);
    	        		ToggleLed(PIN_2);
    	        		break;
    	    	case 3:
    	    		printf("%d:Blue led toggle 3times\n",choice);
			fflush(stdout);
    	        		ToggleLed(PIN_3);
    	        		break;
    	    	case 4:
				printf("%d:Toggle all leds\n",choice);
				fflush(stdout);
    	        		TurnOnLeds();
    	        		break;
    	    	default:
