...
int main(void) {
	...
	tty.c_cflag &= ~PARENB; 	/* Clear parity bit, disabling parity (most common) */
 	...
 	printf("The settings for ttyS0(UART1) serial port is saved\n");
	...
	printf("%d bytes of data received from host:%s\n",RecvBytes,RecvBuff);
 	...
 	*/
 	char WriteBuff[BUF_SIZE] = "Hii host,This is Raspberry pi target! ";
 	if(write(SerialPort,WriteBuff, sizeof(WriteBuff)) > 0){
	 	printf("\nData sent back to host: %s\n",WriteBuff);
	}
	else{
