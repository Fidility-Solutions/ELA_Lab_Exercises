...
int main(void) {
	...
	printf("This program will demonistrate UART1 Tx & Rx over the ttyUSB0 from hhost to target and vice cersa\n");
	...
	printf("ttyUSB0 device file opened\n");
  	...
  	printf("The parameters are set for ttyUSB0 \n");
  	...
  	char WriteBuff[BUF_SIZE] =  "Hello, Target! this is ubuntu host";
	...
	if(WriteBytes > 0){
 	 ...
 	 RecvBytes = read(SerialPort, &RecvBuff, sizeof(RecvBuff));
