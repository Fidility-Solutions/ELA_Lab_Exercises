...
int main(void) {
   	 ...
   	 char TxBuffer[BUFFER_SIZE] = "Hello UART1! This is sample test code for data tx & rx over uart1 loopback\n";
   	...
   	UartFileDisc = open(UART_DEVICE, O_RDWR | O_NOCTTY);
   	 ...
   	 printf("The ttyS0 Device file opened successfully\n");
   	 ...
   	 options.c_cc[VTIME] = 200;          		/* Wait for up to 2s (20 deciseconds), returning as soon as any data is received. */
	 ...
	 printf("The new serial port options set to ttyS0\n");
		...
		printf("%ld bytes of data received successfully on over UART1:%s\n",RecvBytes,RxBuffer);
    	} 
	else {
